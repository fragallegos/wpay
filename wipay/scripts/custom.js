...
$(document).ready(function(){
	...
	function init_template(){
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        $('.input-required input[type="textarea"]').on('focusout',function(){if (textareaValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(required)");}else{$(this).parent().find('em').html(invalidIcon);}}});
        ...
        function highlight_colors(){
            var bodyColor = 'none'
            if($('body').data('highlight') != readCookie('sticky-color-scheme')){var bodyColor = readCookie('sticky-color-scheme');}
            if(readCookie('sticky-color-scheme') == null){ var bodyColor = $('body').data('highlight');}

            var bodyBackground = 'none'
            if($('body').data('background') != readCookie('sticky-bg-scheme')){var bodyBackground = readCookie('sticky-bg-scheme');}
            if(readCookie('sticky-bg-scheme') == null){var bodyBackground = $('body').data('background');}

            if(isDevelopment == true){var bodyColor = $('body').data('highlight'); var bodyBackground = $('body').data('background');}
...
$("#getInfoButton2").click(function(event) {
  event.preventDefault();
  alert("hola");
  console.log('jdhsksuewd111335d5s8sd5X');
  var nombre = $( "input[type=name][name=Name]" ).val();
  var email =  $( "input[type=email][name=emailCostumer]" ).val();
  var ubicacion =  $( "input[type=textarea][name=ubicacion]" ).val();
  var tipoId =  $( "select[name=idtype]" ).val();
  var cedula =  $( "input[type=tel][name=cedula]" ).val();
  var celular = $( "input[type=tel][name=numero]" ).val();
  ...
  var data = {
    ...
    items: [
          {
            producto: 'Reloj Moon',
            descripcion: 'Reloj de la mejor calidad',
            precio: '0.50',
            cantidad: '1'
          }
        ]
  ...
  if(data.nombre && data.email && data.celular && data.billing && data.cedula){

    //var url = 'https://infinityec.co/wipay/beackend/prueba.php';
    var url2 = 'https://wipayservicios.herokuapp.com/nuevaVenta'
      ...
      .then(function (response) {
        ...
        generarClaveDatafast(query.res.id, query.customerid);
      })
      ...
      var generarClaveDatafast = function(id, custumerId){
        console.log(id);
        var s = document.createElement("script");
        s.type = "text/javascript";
        s.src = `https://test.oppwa.com/v1/paymentWidgets.js?checkoutId=${id}`;
        let date = new Date(Date.now() + 86400e3);
        date = date.toUTCString();
        document.cookie = "custumerId="+custumerId+"; expires=" + date;
        //https://infinityec.co/wipay/wipay/page-sucess.html   https://infinityec.co/wipay/beackend/checkout.php https://infinityec.co/wipay/beackend/checkout.php
        var form = `<form action="https://infinityec.co/wipay/wipay/success-error-payment.php" class="paymentWidgets" data-brands="VISA MASTER">`;
        $("head").append(s);
        $("#TarjetaFormulario").html(form);
      }
